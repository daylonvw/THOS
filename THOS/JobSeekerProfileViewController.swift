...
//                        let bluredImage = UIImage(data: data!)?.applyBlurWithRadius(20, tintColor: tintColor, saturationDeltaFactor: 2.0, maskImage: nil)
//                        
//                        self.backgroundImageView.image = bluredImage
    ...
    @IBAction func switchUserTypeVCButtonPressed(sender: AnyObject) {
        
        PFUser.currentUser()?.setValue("seeker", forKey: "userType")
        PFUser.currentUser()?.saveInBackground()

        
        let storyBoard = UIStoryboard(name: "Main", bundle: nil)
        let controller = storyBoard.instantiateViewControllerWithIdentifier("helpSeekertabBarVC")
